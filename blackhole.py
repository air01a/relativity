...
def lens_equation(theta, mass, d_lens, d_source):
    ...
    return theta - (rs / np.maximum(theta, 1e-10)) * (d_source - d_lens) / d_source
...
def raytrace_channel(channel, mass, d_lens, d_source, scale):
    ...
    valid = theta > 1e-10
    schwarzschild = theta< 0.00081362/3

    beta_x = np.zeros_like(theta_x)
    beta_y = np.zeros_like(theta_y)

    # Recalculate projection on image
    ...
    deformed_channel = map_coordinates(channel, [beta_y_pix.flatten(), beta_x_pix.flatten()], order=3, mode='wrap').reshape(height, width)
...
d_lens = 0.5e20  # Distance to object
d_source = 1e20  # Distance to source
scale = 1e-6  # Radians seen for 1 pixel, equivalent for 1 arcsecond (1/(60*60)*2pi/180)
